...
For CircuitNet-N28, the features are extracted after routing. For CircuitNet-N14, the features are extracted 2 times, after CTS (clock tree synthesis) and after routing, respectively (post_place is removed due to inconsistent pin position). They can all be used in net delay prediction.
...
The dictionary has pin names as keys, and a list as value. The list contains 2 features, which are pin positions in micron and pin positions on gcell grid. The pin positions have 4 values, which are the four corners of the bounding box.
