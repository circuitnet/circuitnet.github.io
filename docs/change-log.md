---
title: "Change Log"
---

# Change Log

## 2026/05/17

::: info Major Updates
- Add CircuitNet 3.0 dataset ([Paper](https://openreview.net/forum?id=lEDb4gQ4dB) [Link](https://huggingface.co/datasets/SKLP-EDA-LAB/CircuitNet3.0))!
:::

## 2024/11/09

::: info Major Updates
- Re-uploaded LEF/DEF, netlist and graph information to address issue #38
- Added graph construction demo using the graph information in the dataset
- Introduced FAQ section on the documentation website
:::

## 2023/7/24

::: info Feature Extraction
Released code for feature extraction, enabling users to:

- Implement custom features using the provided LEF/DEF
- Extract features from external LEF/DEF sources

For detailed documentation, please refer to the [README](https://github.com/circuitnet/CircuitNet/blob/main/feature_extraction/README.md).
:::

## 2023/6/29

::: info Net Delay Prediction
- Released code for net delay prediction
- Added comprehensive tutorial on net delay prediction
  - Available at: [Documentation](https://circuitnet.github.io/tutorial/experiment_tutorial.html#Net_Delay)
:::

## 2023/6/14

::: info Dataset Expansion
- Renamed original dataset to **CircuitNet-N28**
  - Added timing features
- Released **CircuitNet-N14**
  - Supports congestion prediction
  - Supports IR drop prediction
  - Supports timing prediction
:::

## 2023/3/22

::: info ISPD2015 Integration
- Updated LEF/DEF with sanitized technology information
- Added ISPD2015 benchmark features:
  - Congestion features
  - Graph features
  - Available in ISPD2015 directory
  - Distribution: [Google Drive](https://drive.google.com/drive/u/1/folders/1GjW-1LBx1563bg3pHQGvhcEyK2A9sYUB) | [Baidu Netdisk](https://pan.baidu.com/disk/main#/index?category=all&path=%2Fapps%2Fbypy%2FCircuitNet)
:::

## 2022/12/29

::: info LEF/DEF Release
- Released sanitized LEF/DEF files
- Available in LEF&DEF directory
- Distribution: [Google Drive](https://drive.google.com/drive/u/1/folders/1GjW-1LBx1563bg3pHQGvhcEyK2A9sYUB) | [Baidu Netdisk](https://pan.baidu.com/disk/main#/index?category=all&path=%2Fapps%2Fbypy%2FCircuitNet)
:::

## 2022/12/12

::: info Graph Features
- Released graph features
- Available in graph_features directory
- Distribution: [Google Drive](https://drive.google.com/drive/u/1/folders/1GjW-1LBx1563bg3pHQGvhcEyK2A9sYUB) | [Baidu Netdisk](https://pan.baidu.com/disk/main#/index?category=all&path=%2Fapps%2Fbypy%2FCircuitNet)
:::

## 2022/9/6

::: info Model Weights
- Released pretrained model weights
- Distribution: [Google Drive](https://drive.google.com/drive/folders/10PD4zNa9fiVeBDQ0-drBwZ3TDEjQ3gmf?usp=sharing) | [Baidu Netdisk](https://pan.baidu.com/s/1dUEt35PQssS7_V4fRHwWTQ?pwd=7i67)
:::

## 2022/8/1

::: info Initial Release
- First public release of CircuitNet
:::
